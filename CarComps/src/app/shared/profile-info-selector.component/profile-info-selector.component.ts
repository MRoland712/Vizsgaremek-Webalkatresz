...
export class ProfileInfoSelectorComponent implements OnInit {
  ...
  ProfileDatas        = signal({ id: 0, firstname: '', lastname: '', username: '', email: '', phone: '' });
  addressId           = signal<number>(0);
  savedAddressSummary = signal<string>('Cím kezelése');
  ...
  // ── Lifecycle ─────────────────────────────────────────────
  ngOnInit() {
    if (localStorage.getItem('tfaActive') === 'true') this.isTfaActive.set(true);
    ...
    this.loadAddressSummary(userId);

    if (userId > 0) { this.loadUserById(userId); return; }

    if (email) {
      const token = localStorage.getItem('jwt') ?? '';
      const headers = new HttpHeaders({ token });
      this.http.get<any>(`${this.baseUrl}user/getUserByEmail`, { params: { email }, headers }).subscribe({
        next: (res) => {
          ...
          if (uid) {
            ...
            this.auth.setUserProfile({
              ...
              username: u.username || '', email: u.email || email,
  ...
  // ── Cím localStorage segédek ──────────────────────────────
  private addressKey(userId: number): string {
    return `shippingAddress_${userId}`;
  }

  private loadAddressSummary(userId: number) {
    if (!userId) return;
    const saved = localStorage.getItem(this.addressKey(userId));
    if (!saved) return;
    try {
      const addr = JSON.parse(saved);
      if (addr.city && addr.street) {
        this.savedAddressSummary.set(`${addr.city}, ${addr.street}`);
      }
    } catch {}
  }

  private saveAddressToStorage(userId: number, data: any) {
    localStorage.setItem(this.addressKey(userId), JSON.stringify(data));
    if (data.city && data.street) {
      this.savedAddressSummary.set(`${data.city}, ${data.street}`);
    }
    localStorage.setItem('shippingAddress', JSON.stringify({ ...data, _userId: userId }));
  }
    ...
    this.getUserByIdSvc.getUserById(userId).subscribe({
      next: (res) => {
        ...
        this.auth.setUserProfile({
          ...
          username: u.username, email: u.email, phone: u.phone, role: u.role,
        });
        this.ProfileDatas.set({
          id: u.id, firstname: u.firstName, lastname: u.lastName,
      ...
      error: (err) => {
        ...
        this.ProfileDatas.set({
          id: 0, firstname: this.auth.userFirstName(), lastname: this.auth.userLastName(),
          username: this.auth.userName(), email: this.auth.userEmail(), phone: this.auth.userPhone(),
        });
        this.isLoadingUser.set(false);
      },
    });
  ...
  openDialog(field: EditField) {
    ...
    switch (field) {
      case 'fullname':  this.editForm.patchValue({ firstname: d.firstname, lastname: d.lastname }); break;
      case 'username':  this.editForm.patchValue({ username: d.username }); break;
      case 'email':     this.editForm.patchValue({ email: d.email }); break;
      case 'phone':     this.editForm.patchValue({ phone: d.phone }); break;
      case 'password':  this.editForm.patchValue({ currentPassword: '', newPassword: '', confirmPassword: '' }); break;
      case '2fa':       this.open2FADialog(); break;
      case 'orders':    this.loadOrders(); break;
      case 'address':   this.loadAddress(); break;
    }
  }

  private open2FADialog() {
    if (this.isTfaActive()) { this.tfaStep.set('disable'); this.disableError.set(null); }
    else this.initiate2FA();
  }
  ...
  onSave() {
    ...
    const relevantControls: Record<string, string[]> = {
      fullname: ['firstname', 'lastname'], username: ['username'], email: ['email'],
      password: ['newPassword'], phone: ['phone'],
      address: ['country', 'city', 'postalCode', 'street'],
    };
    const isInvalid = (relevantControls[field] ?? []).some((key) => {
    ...
    this.saveError.set(null);
    switch (field) {
      case 'fullname': this.updateFullName(); break;
      case 'username': this.updateUsername(); break;
      case 'email':    this.updateEmail();    break;
      case 'password': this.updatePassword(); break;
      case 'phone':    this.updatePhone();    break;
      case 'address':  this.updateAddress();  break;
    }
  }
  ...
  onTFACancelledForSave() { this.pendingField.set(null); }
  ...
  private updateFullName() {
    ...
    const body = { firstName: this.editForm.value.firstname!, lastName: this.editForm.value.lastname! };
    this.updateUserSvc.updateUserInfos(email, body).subscribe({
      next: () => {
        this.ProfileDatas.update(d => ({ ...d, firstname: body.firstName, lastname: body.lastName }));
        this.auth.setUserProfile({ ...this.currentProfile(), firstName: body.firstName, lastName: body.lastName });
        this.handleSuccess();
      },
  ...
  private updateUsername() {
    ...
    this.updateUserSvc.updateUserInfos(email, body).subscribe({
      next: () => { this.ProfileDatas.update(d => ({ ...d, username: body.username })); this.handleSuccess(); },
      error: (err) => this.handleError(err),
    });
  }
  ...
  private updateEmail() {
    ...
    this.updateUserSvc.updateUserInfos(oldEmail, body).subscribe({
      next: () => { this.ProfileDatas.update(d => ({ ...d, email: body.email })); this.handleSuccess(); },
      error: (err) => this.handleError(err),
    });
  }
  ...
  private updatePassword() {
    if (this.editForm.value.newPassword !== this.editForm.value.confirmPassword) {
      this.saveError.set('Az új jelszavak nem egyeznek!'); this.isSaving.set(false); return;
    }
    const email = this.ProfileDatas().email;
    const body = { currentPassword: this.editForm.value.currentPassword!, newPassword: this.editForm.value.newPassword! };
    this.updateUserSvc.updateUserInfos(email, body).subscribe({
  ...
  private updatePhone() {
    ...
    this.updateUserSvc.updateUserInfos(email, body).subscribe({
      next: () => { this.ProfileDatas.update(d => ({ ...d, phone: body.phone })); this.handleSuccess(); },
      error: (err) => this.handleError(err),
    });
  }
  ...
  private updateAddress() {
    ...
    if (!addrId) { this.createAddress(); return; }
    ...
    this.updateAddressSvc.updateAddressInfos(addrId, body).subscribe({
      next: () => {
        if (newPhone !== this.ProfileDatas().phone) {
          ...
          this.updateUserSvc.updateUserInfos(this.ProfileDatas().email, { phone: newPhone }).subscribe();
        }
        this.saveAddressToStorage(userId, {
  ...
  private createAddress() {
    const userId = this.ProfileDatas().id || this.auth.userId();
    if (!userId) { this.saveError.set('Nem sikerült azonosítani a felhasználót!'); this.isSaving.set(false); return; }
  ...
  private currentProfile() {
    const d = this.ProfileDatas();
    return { id: d.id, firstName: d.firstname, lastName: d.lastname, username: d.username, email: d.email, phone: d.phone };
  }
  ...
  initiate2FA() {
    ...
    this.tfaService.CreateUserTfa({ email: this.ProfileDatas().email }).subscribe({
      next: (res) => { this.tfaData.set(res); this.isLoadingTFA.set(false); },
      error: (err) => {
        this.isLoadingTFA.set(false);
        if (err.status === 409 && err.error?.errors?.includes('UserHasActiveTFA')) {
          this.isTfaActive.set(true); localStorage.setItem('tfaActive', 'true'); this.tfaStep.set('disable'); return;
        }
  ...
  disableTFA() {
    ...
    if (!userId) { this.disableError.set('Nem található a felhasználó azonosítója!'); this.isDisabling.set(false); return; }
    this.tfaService.getUserTwofa(userId).subscribe({
      next: (res) => {
        const tfaId = res.result.id;
        this.tfaService.disableTfa(tfaId).subscribe({
          next: () => { this.isTfaActive.set(false); localStorage.removeItem('tfaActive'); this.isDisabling.set(false); this.closeDialog(); },
          error: (err) => { this.disableError.set(err.error?.message || err.error?.errors?.[0] || 'Hiba a 2FA kikapcsolásakor'); this.isDisabling.set(false); },
        });
      },
      error: (err) => { console.error('❌ getUserTwofa hiba:', err); this.disableError.set('Nem sikerült lekérdezni a 2FA adatokat'); this.isDisabling.set(false); },
    });
  }

  toggleSecretKey() { this.showSecretKey.update(v => !v); }
  copyToClipboard(text: string) { navigator.clipboard.writeText(text).then(() => alert('Másolva!')); }
  ...
  verifyCode() {
    ...
    if (code.length !== 6) { this.verificationError.set('Add meg a teljes 6 számjegyű kódot'); return; }
    this.isVerifying.set(true);
    this.verificationError.set(null);
    this.tfaService.verifyTfaCode(this.ProfileDatas().email, code).subscribe({
      next: (res) => {
        this.isVerifying.set(false);
        if (res.result === 'invalid') { this.verificationError.set('Hibás kód! Ellenőrizd az authenticator alkalmazást.'); return; }
        this.tfaStep.set('recovery');
      },
      error: (err) => {
        this.isVerifying.set(false);
        if (err.status === 404 && err.error?.errors?.includes('UserTwofaNotFound')) { console.warn('⚠️ TFA 404 bypass'); this.tfaStep.set('recovery'); return; }
  ...
  downloadRecoveryCodes() {
    ...
    a.href = url; a.download = `carcomps-2fa-${Date.now()}.txt`; a.click();
    URL.revokeObjectURL(url);
  }

  copyAllRecoveryCodes() {
    const codes = this.tfaData()?.result?.recoveryCodes || [];
    navigator.clipboard.writeText(codes.map((c, i) => `${i + 1}. ${c}`).join('\n')).then(() => alert('Másolva!'));
  }

  finish2FASetup() { this.isTfaActive.set(true); localStorage.setItem('tfaActive', 'true'); this.closeDialog(); }

  // ── Rendelések ────────────────────────────────────────────
  private loadOrders() {
    const userId = this.ProfileDatas().id || this.auth.userId() || Number(localStorage.getItem('userId') || '0');
    if (!userId) { this.ordersError.set('Nem sikerült azonosítani a felhasználót!'); return; }
    this.isLoadingOrders.set(true);
    this.ordersError.set(null);
    this.orderService.getOrderByUserId(userId).subscribe({
      next: (res) => { this.orders.set(res.orders ?? []); this.isLoadingOrders.set(false); },
      error: (err) => { console.error('❌ getOrderByUserId hiba:', err); this.ordersError.set('Nem sikerült betölteni a rendeléseket.'); this.isLoadingOrders.set(false); },
    });
  }

  // ── Segédek ───────────────────────────────────────────────
  private handleSuccess() { this.isSaving.set(false); this.saveSuccess.set(true); setTimeout(() => this.closeDialog(), 2000); }
  private handleError(err: any) { console.error('❌ Mentés hiba:', err); this.isSaving.set(false); this.saveError.set(err.error?.message || 'Hiba történt a mentés során'); }

  getDialogTitle(): string {
    const map: Record<string, string> = {
      fullname: 'Teljes név szerkesztése', username: 'Felhasználónév szerkesztése',
      email: 'Email cím szerkesztése', password: 'Jelszó módosítása',
      phone: 'Telefonszám szerkesztése', '2fa': 'Kétfaktoros hitelesítés',
      orders: 'Rendeléseim', address: 'Szállítási cím',
    };
  ...
  getOrderStatusClass(status: string): string {
    switch (status.toLowerCase()) {
      case 'completed': case 'delivered': case 'paid': return 'status-success';
      case 'pending':   case 'processing':              return 'status-warning';
      case 'cancelled': case 'failed':                  return 'status-error';
