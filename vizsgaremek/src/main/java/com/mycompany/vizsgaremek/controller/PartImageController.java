...
public class PartImageController {
    ...
    @GET
    @Path("/part/{partId}")
    public Response getPartImagesByPartId(@PathParam("partId") Integer partId) {
        try {
            // Validáció
    ...
    @GET
    @Path("/{id}")
    public Response getPartImageById(@PathParam("id") Integer id) {
        try {
            // Validáció
            if (id == null || id <= 0) {
                JSONObject error = new JSONObject();
    ...
    @Path("/{id}")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response updatePartImage(@PathParam("id") Integer id, String body) {
        try {
            // Validáció
            if (id == null || id <= 0) {
                JSONObject error = new JSONObject();
    ...
    @DELETE
    @Path("/{id}")
    public Response deletePartImage(@PathParam("id") Integer id) {
        try {
            // Validáció
            if (id == null || id <= 0) {
                JSONObject error = new JSONObject();
            ...
            // Opcionális: Fizikai fájl törlése is
    ...
    /**
     * Helper - Fájlnév kinyerése a header-ből
     */
